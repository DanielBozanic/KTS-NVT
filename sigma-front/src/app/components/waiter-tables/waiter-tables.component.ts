...
export class WaiterTablesComponent implements OnInit {
  ...
  reserveTable(id: number){
    this.service.changeTableState(id, 'RESERVED', 1234).subscribe( (data) => this.getTables(this.zoneId));
  ...
  pay(id : number){
    this.service.changeTableState(id, 'FREE', 1234).subscribe( (data) => this.getTables(this.zoneId));
    this.paymentTableDialog.closeAll();
  }
    ...
    switch(table.state){
      ...
      case 'IN_PROGRESS':
        if(this.tables[0].orderId){
          this.service.getItemsForOrder(this.tables[0].orderId).subscribe( (data) => {
            this.currentItems = data;
            const dialogref = this.tableOrderDialog.open(this.orderDialog, {height: '80%', width: '50%'});
          })
        }
        break;
